...
if (show_interpretation) {
  cat(" Implications for OLS Estimators
      the coef( the betas ) are not normally distr in small              samples
       var of the coef changes , we have a more robust variance V       formula if we use OLS
      OLS is no longer effecient , we use UGLS wich has a lower       variance of the estimator
     How might heteroskedasticity distort inference about the
    return to education 
    the st errors of edc are underestimated and are not to be         trusted")
}
```

```{r Adress-heterskedasticity robust-st,  echo=TRUE,results="asis"}

robust_se <- sqrt(diag(vcovHC(linear_model1, type = "HC1")))
stargazer(linear_model1, type = "text", se = list(robust_se), title = "Robust Standard Errors")
if (show_interpretation) {
  cat("we see an increase in st errors of OLS , as expected bcs we have heteroskedasticity")
}
