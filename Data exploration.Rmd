...
```{r EGLS estimation, echo=TRUE}
# Step 1: Get design matrix and response variable
X <- model.matrix(linear_model1)  # Includes intercept & region dummies
Y <- df$WAGE

# Step 2: Get initial residuals from baseline model
resid_init <- residuals(linear_model1)

# Step 3: Create y_bar (mean of Y)
y_bar <- mean(Y)

# Step 4: Transform X and Y
X_star <- X / y_bar
Y_star <- Y / y_bar  # Element-wise division

# Step 5: EGLS estimation (no intercept needed as it's in X_star)
egls_model <- lm(Y_star ~ X_star - 1)  # -1 removes additional intercept

stargazer(egls_model,
          type = "latex",
          title = "EGLS Estimation Results",
          dep.var.labels = "Transformed WAGE (Y/ȳ)",
          covariate.labels = c("Intercept", "EDUC", "AGE", "RACE", "SMSA", "MARRIED",
                             "REGION2", "REGION3", "REGION4", "REGION5",
                             "REGION6", "REGION7", "REGION8", "REGION9"),
          notes = "Variables transformed by dividing by mean wage (ȳ)",
          notes.append = TRUE,
          digits = 4)
