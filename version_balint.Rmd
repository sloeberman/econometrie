...
cat("# **Step 1: Specification, hypotheses, and descriptive statistic**\n\n")
```
...
cat("# **Step 2: OLS estimation and testing**\n\n")
```
...
cat("# **Step 3: Checking GM assumptions and individual remedies**\n\n")
if (show_interpretation) {
...
```{r LM test educ en age, echo=FALSE,results='asis'}
...
```
```{r LM test educ, echo=FALSE,results='asis'}
res=linear_model1$residuals
...
```{r LM test age, echo=FALSE,results='asis'}
res=linear_model1$residuals
...
names(LM_summary)=c("Test-statistic","P-value")
stargazer(LM_summary,type="text")
if (show_interpretation){cat("The test results suggest that the omitted non-linearities are due to EDUC but not due to AGE -> Try adding EDUC² and EDUC³ to the baseline.")}
```
...
```{r RESET log, echo=FALSE,results='asis'}
...
```{r best model reset, echo =FALSE,results = 'asis'}
