...
REGION8 = as.integer(REGION == 8),
REGION9 = as.integer(REGION == 9))
best_mod <- lm(WAGE~ EDUC + I(EDUC^2) + I(EDUC^3)+I(EDUC^4) + AGE + RACE + I(EDUC*RACE) +SMSA + MARRIED + REGION2 + REGION3 + REGION4 + REGION5 + REGION6 + REGION7 + REGION8 + REGION9, data = df)
beta=best_mod$coefficients
X_hold <- model.matrix(WAGE~ EDUC + I(EDUC^2) + I(EDUC^3)+I(EDUC^4) + AGE + RACE + I(EDUC*RACE) +SMSA + MARRIED + REGION2 + REGION3 + REGION4 + REGION5 + REGION6 + REGION7 + REGION8 + REGION9,data = hold)
y_hold <- hold$WAGE
...
```{r, echo=FALSE, results="asis"}
if (show_interpretation) {
  cat("\n The p-value is 1, so we can't reject the null hypothesis, in other words, there's no misspecification. But we also tried this test with log(Wage) because previously we found that it improved the fit of the model, however this test showed that there's still misspecification. This is the reason that we have left out log(wage).\n")
