...
# Assuming your data frame is named 'data' (not 'df' as in your quantile calculations)
# Fix this inconsistency first:
lower_bound <- quantile(log(data$WAGE), 0.005, na.rm = TRUE)
upper_bound <- quantile(log(data$WAGE), 0.995, na.rm = TRUE)

# Convert to factors and drop unused levels
data$QOB <- droplevels(factor(data$QOB))
data$REGION <- droplevels(factor(data$REGION))

# Check factor levels (must be >= 2)
stopifnot(nlevels(data$QOB) >= 2, nlevels(data$REGION) >= 2)

# Filter on log(WAGE) - make sure to use the same data frame name
data_trimmed <- data %>% filter(log(WAGE) >= lower_bound, log(WAGE) <= upper_bound)

# 2SLS regression with explicit interactions
# Important: Use the trimmed dataset here!
iv_model <- ivreg(
  log(WAGE) ~ EDUC + I(EDUC^2) + I(EDUC^3) + EDUC:RACE + AGE + RACE + MARRIED + SMSA + REGION | 
  QOB + QOB:RACE + AGE + RACE + MARRIED + SMSA + REGION+QOB^2+QOB^3,  # Instruments
  data = data_trimmed  # Use the filtered data
)


# Compute HAC robust variance-covariance matrix
robust_se <- vcovHAC(iv_model)

# Apply robust SE to the model coefficients
